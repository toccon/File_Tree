...
public class TestFileTree {
	...
	private static void test9_10(int test) {
		...
		String[] output10 = {"folder3/folderA/folderB", "folder3/folderB",
				"folder3/folderA/folderD", "folder3/folderC/folderE/folderD",
				"folder3/folderA/folderD/folderF", "folder3/folderC/folderE/folderD/folderF"
		};
		...
		if (test == 9) output = output9;
		else output = output10;
		...
		try {
			...
			if (FileObject.testFailed())
				...
				else list = tree.duplicatedFolders();

				// Check that files/folders with the same name appear in consecutive positions of the iterator
				while (list.hasNext()) {
					s = list.next();
					s1 = list.next();
				...
				if (index < output.length || list.hasNext()) testPassed = false;
